...
XtrainSC = model.transform(Xtrain)
np.savetxt('SC_' + name + '_TRAIN.csv', XtrainSC, delimiter=",")

XtestSC = model.transform(Xtest)
np.savetxt('SC_' + name + '_TEST.csv', XtestSC, delimiter=",")
...
Xtrain_hat = Xtrain_transformed @ model.components_
L = 0.5*np.linalg.norm(Xtrain-Xtrain_hat)**2

# Variance explained
SST = np.sum(Xtrain**2)
VE_SC = 1-2*L/SST 

print('SC variance explained, Xtrain:', VE_SC)

...
model = decomposition.PCA(n_components=128)
model.fit(Xtrain)
...
XtrainSVD = model.transform(Xtrain)
np.savetxt('SVD_' + name + '_TRAIN.csv', XtrainSVD, delimiter=",")
XtrainSVD = None

XtestSVD = model.transform(Xtest)
np.savetxt('SVD_' + name + '_TEST.csv', XtestSVD, delimiter=",")
